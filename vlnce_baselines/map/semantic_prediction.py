...
class GroundedSAM(Segment):
    ...
    @torch.no_grad()
    def detect(self, image: VisualObservation, **kwargs) -> Tuple[List[str], sv.Detections]:
        """Run GroundingDINO only (no SAM), returning labels and detections.

        This is useful when we only need a textual list of detected objects
        (e.g., for VLM prompting) and want to avoid drawing points/overlays.
        """
        classes = kwargs.get("classes", [])
        labels: List[str] = []
        ...
        for _, _, confidence, class_id, _ in detections:
            if class_id is not None and 0 <= int(class_id) < len(classes):
                labels.append(f"{classes[int(class_id)]} {float(confidence):0.2f}")
            else:
                labels.append(f"unknow {float(confidence):0.2f}")

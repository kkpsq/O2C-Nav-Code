...
class FusionMapPolicy(nn.Module):
    ...
    def _get_action(self, 
        ...
        if self.visualize:
            ...
            #cv2.waitKey(1)
        if self.visualize:
            save_dir = os.path.join(self.config.RESULTS_DIR, "fmm_fields/eps_%s"%current_episode_id)
    ...
    def _visualization(self, 
        ...
        if self.print_images:
            save_dir = os.path.join(self.config.RESULTS_DIR, "waypoints/eps_%s"%current_episode_id)
            os.makedirs(save_dir, exist_ok=True)
            fn = "{}/step-{}.png".format(save_dir, step)
